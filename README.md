## 📂 Projects

### 💰 Expense Tracker
A Python application that tracks expenses and calculates spending patterns.

### 📄 AI Resume Analyzer
Analyzes candidate skills, calculates match percentage, and suggests improvements.

### 🎤 AI Interview Generator
Generates interview questions and helps users prepare for technical interviews.

### 🎓 AI Student Profile Analyzer
Collects student information, analyzes performance, and provides career recommendations.

### 💾 Smart AI Student Database System
Stores student information, performs academic analysis, and saves records using file handling.

### 🎯 AI Career Skill Gap Analyzer V2
Analyzes user skills, identifies missing skills, calculates career readiness, and generates personalized learning roadmaps.

### 🎯 AI Placement Predictor & Career Advisor 
An AI-inspired placement prediction system built using Python that evaluates a student's academic performance and technical skills to estimate placement readiness.

### 📊 AI Smart Attendance & Performance Analyzer 
An AI-inspired student analytics system built with Python that evaluates attendance and academic performance to generate intelligent insights.

